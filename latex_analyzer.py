...
class TestAnalyzer:
    ...
    def load_data(self):
        ...
        try:
            ...
            if self.local_log_path and self.ground_truth_csv:
                print("\n🔄 Integrando datos del modo local...")
    ...
    def _integrate_local_logs(self):
        ...
        if not self.local_parser.parse_log():
            print("⚠️ No se pudieron integrar los logs locales")
            return
...
\end{table}"""
    ...
    def generate_report(self):
        ...
        # Si hay parser local, mostrar sus métricas
        if self.local_parser and self.local_parser.results:
            print("\n" + "="*60)
            print("📊 ANÁLISIS DEL MODO LOCAL")
            print("="*60)
        ...
        print(f"\n✅ CSV integrado guardado: {output_csv}")
...
if __name__ == "__main__":
    ...
    # Configurar paths
    csv_hibrido = "output_plots/resultados_integrados.csv"
    ...
    if not has_hibrido and not has_local:
        ...
        if has_local:
            print("\n✅ Modo: ANÁLISIS COMPLETO (Local + Híbrido)")
